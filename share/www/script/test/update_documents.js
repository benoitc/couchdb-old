...
"LvEliY253nA3pVhQqdPt0f/erJkMGMB8xucAAAAASUVORK5CYII="].join(''),
  ...
  // update error
  var xhr = CouchDB.request("GET", "/test_suite_db/_design/template/_update/");
  T(xhr.status == 404, 'Should be missing');
  T(JSON.parse(xhr.responseText).reason == "Invalid path.");
  
  // hello template world
  xhr = CouchDB.request("GET", "/test_suite_db/_design/template/_update/hello/"+docid);
  ...
  // // error stacktraces
  // xhr = CouchDB.request("GET", "/test_suite_db/_design/template/_update/render-error/"+docid);
  // T(JSON.parse(xhr.responseText).error == "render_error");
 
  // hello template world (no docid)
  xhr = CouchDB.request("GET", "/test_suite_db/_design/template/_update/hello");
  T(xhr.responseText == "Empty World");

  // // hello template world (non-existing docid)
  xhr = CouchDB.request("GET", "/test_suite_db/_design/template/_update/hello/nonExistingDoc");
  T(xhr.responseText == "New World");
  
  // update with doc
  xhr = CouchDB.request("GET", "/test_suite_db/_design/template/_update/just-name/"+docid);
  T(xhr.responseText == "Just Rusty");
  
  // update with missing doc
  xhr = CouchDB.request("GET", "/test_suite_db/_design/template/_update/just-name/missingdoc");

  T(xhr.status == 404, 'Doc should be missing');
  T(xhr.responseText == "No such doc");
  
  // update with missing func
  xhr = CouchDB.request("GET", "/test_suite_db/_design/template/_update/missing/"+docid);
  T(xhr.status == 404, "function is missing");
  
  // missing design doc
  xhr = CouchDB.request("GET", "/test_suite_db/_design/missingddoc/_update/just-name/"+docid);
  T(xhr.status == 404);
  var resp = JSON.parse(xhr.responseText);
  T(resp.error == "not_found");

  // query parameters
  xhr = CouchDB.request("GET", "/test_suite_db/_design/template/_update/req-info/"+docid+"?foo=bar", {
    headers: {
  ...
  xhr = CouchDB.request("GET", "/test_suite_db/_design/template/_update/xml-type/"+docid);
  ...
  xhr = CouchDB.request("GET", "/test_suite_db/_design/template/_update/accept-switch/"+docid, {
  ...
  designDoc.updates["just-name"] = (function(doc, req) {
   return {
  ...
  // JS can't set etag
  xhr = CouchDB.request("GET", "/test_suite_db/_design/template/_update/no-set-etag/"+docid);
  // extract the ETag header values
  etag = xhr.getResponseHeader("etag");
  T(etag != "skipped")

  // test the respondWith mime matcher
  xhr = CouchDB.request("GET", "/test_suite_db/_design/template/_update/respondWith/"+docid, {
    headers: {
  ...
  xhr = CouchDB.request("GET", "/test_suite_db/_design/template/_update/json/foo");
